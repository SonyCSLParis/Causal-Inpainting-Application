...
class Attention_(nn.Module):
    ...
    def forward(self,
        ...
        q, k, v, theta_q = map(
            ...
            if t is not None else None, (q, k, v, theta_q))
        # split between global and local heads
        (q, lq), (k, lk), (v, lv), (theta_q, ltheta_q) = map(
            lambda t: (t[:, :gh], t[:, gh:])
            if t is not None else (None, None), (q, k, v, theta_q))
...
def get_pes(layer_pe, n_global_heads, n_local_heads, dataloader_generator):
    ...
    if layer_pe_type == 'rototor':
        layer_pos_emb = Rototor(dim=dim_layerPE,
                                n_heads=n_global_heads,
                                fix=False,
                                init_type=layer_pe_input)
        if n_local_heads > 0:
            layer_pos_emb_local = Rototor(dim=dim_layerPE,
        ...
        layer_pos_emb = Rotary(dim=dim_layerPE,
                               n_heads=n_global_heads,
                               fix=True,
                               init_type=layer_pe_input)
        if n_local_heads > 0:
        ...
        if layer_pe_input == 'index':
            ...
            poscoder = SineSPE(num_heads=n_global_heads,
                               in_features=dim_layerPE,
                               num_sines=num_sines,
                               num_realizations=num_realizations)
            layer_pos_emb = poscoder
            if n_local_heads > 0:
                poscoder_local = SineSPE(num_heads=num_local_head,
                                         in_features=dim_layerPE,
                                         num_sines=num_sines,
                                         num_realizations=num_realizations)
