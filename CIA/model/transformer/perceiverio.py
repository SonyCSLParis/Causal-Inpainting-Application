...
class Encode(nn.Module):
    def __init__(self, dim, num_heads, downscaling, hidden_dim):
        super().__init__()
        self.dim = dim
...
class Process(nn.Module):
    def __init__(self, dim, num_heads, hidden_dim):
        ...
        self.self_attn = Encode(
            dim=dim, num_heads=num_heads, downscaling=1, hidden_dim=hidden_dim
        )
...
class PerceiverIO(nn.Module):
    def __init__(self, dim, num_layers, **kwargs):
        ...
        self.encode = Encode(
            dim=512, num_heads=8, downscaling=self.downscaling, hidden_dim=512
        )
        self.process = nn.Sequential(
            *[Process(dim, num_heads=8, hidden_dim=512) for _ in range(num_layers)]
        )

        # small transformer
        self.post_process = nn.Sequential(
            *[Process(dim, num_heads=8, hidden_dim=512) for _ in range(4)]
        )
    ...
    def forward(self, x, **kwargs):
        ...
        # offset on l
        l = torch.cat([self.dummy_latent.repeat(batch_size, 1, 1), l], dim=1)[:, :-1]
        l = l.reshape(batch_size * num_latents, 1, latent_dim)

        x = x.reshape(
            batch_size * num_tokens // self.downscaling, self.downscaling, feature_dim
        )
