...
class PerceiverIO(nn.Module):
    def __init__(self, dim, num_layers, dropout, num_events, downscaling, **kwargs):
        super().__init__()
        self.dim_last_layer = dim
        latent_dim = dim  # same dim for downscaled transformers

        # which init?
        num_events_latent = num_events // downscaling
        self.l_init = torch.zeros(num_events_latent, latent_dim)
        position = torch.arange(0, num_events_latent, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(
        ...
        self.l_init[:, 1::2] = torch.cos(position * div_term)
        self.l_init = nn.Parameter(self.l_init.unsqueeze(0), requires_grad=False)
        self.dummy_l = nn.ParameterList(
            [nn.Parameter(torch.randn(1, 1, latent_dim)) for _ in range(num_layers)]
        )
        self.write = nn.ModuleList(
            [
                QKV_Write(
        ...
        self.process_l = nn.ModuleList(
            [
                Process(dim=dim, hidden_dim=dim * 2, num_heads=8, dropout=dropout)
                for _ in range(num_layers)
            ]
        )
        ...
        self.norm3 = nn.ModuleList([nn.LayerNorm(dim) for _ in range(num_layers)])
        self.norm_l = nn.ModuleList([nn.LayerNorm(dim) for _ in range(num_layers)])
